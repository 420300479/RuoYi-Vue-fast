...
public class PdfUtil<T> {
    ...
    /**
     * 对list数据源将其里面的数据导入到导入到pdf二进制文件流
     *
     * @param list  导出数据集合
     * @param title 文件表头名称
     * @return 结果
    ...
    public void exportPdf(HttpServletResponse response, List<T> list, String title)
    {
    	...
    	try {
            ...
            input.read(data);
            response.getOutputStream().write(data);} catch (Exception e)
            {
...
//		out = getOutputStream(response, fileName, "pdf");
    ...
    public AjaxResult exportPdf()
    {
        ...
        // 存入一个pdf文件
        //String fileName = setTableData(table);  
        String fileName = encodingFilename(this.title);
        String output = getAbsoluteFile(fileName);
        BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(new File(output)));
        PdfUtil.exportPdf(out, table, "");
        return AjaxResult.success(fileName);} catch (Exception e)
        {
            ...
            if (attr.isExport()) {
                ...
                if (com.ruoyi.common.utils.StringUtils.isNotEmpty(dateFormat) && com.ruoyi.common.utils.StringUtils.isNotNull(value)) {
                    ...
                    if (Excel.ColumnType.STRING == attr.cellType()) {
                        table.addCell(PdfUtil.createCell_1(com.ruoyi.common.utils.StringUtils.isNull(value) ? attr.defaultValue() : value + attr.suffix()));
    ...
    public static PdfPCell createTitleCell_1(String value) {
        ...
        cell.setBackgroundColor(new BaseColor(255, 255, 255)); // 白色
        // cell.setColspan(1);
...
//			不允许编辑 打印 ======
            ...
            stamper.setEncryption(null, ownerPassword, PdfWriter.ALLOW_COPY,PdfWriter.STANDARD_ENCRYPTION_128);
...
//									}
     ...
     */
    public  String mergePdfFile(List<String> sourceFilePaths, String destFilePath) {
        ...
        try {
          /*  // 创建合并后的新文件的目录
            ...
            // 重命名，加上服务器的文件夹路径
            destFilePath = getAbsoluteFile(destFilePath);
    ...
    /**
     * 对list数据源将其里面的数据导入到导入到pdf二进制文件流（多文件）
     *
     * @param list  导出数据集合
     * @param title 文件表头名称
     * @return 结果
